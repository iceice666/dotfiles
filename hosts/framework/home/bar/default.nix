...
let
  ...
  icons = {
    appPlaceholder = pkgs.writeText "framework-bar-app-placeholder.svg" ''
    ...
    '';
    batteryAc = pkgs.writeText "framework-bar-battery-ac.svg" ''
    ...
    '';
    batteryBat = pkgs.writeText "framework-bar-battery-bat.svg" ''
    ...
    '';
    batteryCharging = pkgs.writeText "framework-bar-battery-charging.svg" ''
    ...
    '';
    batteryNormal = pkgs.writeText "framework-bar-battery-normal.svg" ''
    ...
    '';
    batteryUnknown = pkgs.writeText "framework-bar-battery-unknown.svg" ''
    ...
    '';
    brightness = pkgs.writeText "framework-bar-brightness.svg" ''
    ...
    '';
    controlCenter = pkgs.writeText "framework-bar-control-center.svg" ''
    ...
    '';
    network = pkgs.writeText "framework-bar-network.svg" ''
    ...
    '';
    micMuted = pkgs.writeText "framework-bar-mic-muted.svg" ''
    ...
    '';
    notification = pkgs.writeText "framework-bar-notification.svg" ''
    ...
    '';
    speakerHigh = pkgs.writeText "framework-bar-speaker-high.svg" ''
    ...
    '';
    speakerLow = pkgs.writeText "framework-bar-speaker-low.svg" ''
    ...
    '';
    speakerMuted = pkgs.writeText "framework-bar-speaker-muted.svg" ''
    ...
    '';
    tray = pkgs.writeText "framework-bar-tray.svg" ''
    ...
    '';
    bluetooth = pkgs.writeText "framework-bar-bluetooth.svg" ''
    ...
    '';
    clear = pkgs.writeText "framework-bar-clear.svg" ''
    ...
    '';
    darkMode = pkgs.writeText "framework-bar-dark-mode.svg" ''
    ...
    '';
    lock = pkgs.writeText "framework-bar-lock.svg" ''
    ...
    '';
    logout = pkgs.writeText "framework-bar-logout.svg" ''
    ...
    '';
    reboot = pkgs.writeText "framework-bar-reboot.svg" ''
    ...
    '';
    shutdown = pkgs.writeText "framework-bar-shutdown.svg" ''
    ...
    '';
    suspend = pkgs.writeText "framework-bar-suspend.svg" ''
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#000000"><path d="M8 5h3v14H8zM13 5h3v14h-3z"/></svg>
    '';
  };

  barState = lib.getExe pkgs.framework-bar;
  barStateConfig = pkgs.writeText "framework-bar-config.json" (
    builtins.toJSON {
      ...
      commands = {
        ...
        overskride = "${pkgs.overskride}/bin/overskride";
        grep = "${pkgs.gnugrep}/bin/grep";
        jq = "${pkgs.jq}/bin/jq";
        awk = "${pkgs.gawk}/bin/awk";
        lockScreen = toString lockScreen;
      };
      icons = builtins.mapAttrs (_: toString) icons;
    }
  );
  stateCommand = "${barState} --config-file ${barStateConfig}";
in
{
  _module.args = {
    inherit barState barStateConfig;
    barReload = "${stateCommand} reload";
    barNotificationMarkRead = "${stateCommand} notifications mark-read";
    barNotificationMarkUnread = "${stateCommand} notifications mark-unread";
  };

  systemd.user.services.framework-bar = {
    Unit = {
      Description = "Framework bar";
