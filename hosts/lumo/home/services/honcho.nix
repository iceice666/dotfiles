...
let
  ...
  ensureImage = ''
    if ! ${pkgs.podman}/bin/podman image exists ${image}; then
      ${pkgs.podman}/bin/podman build -t ${image} ${honchoSource} >&2
    fi
  '';
  ...
  postgresService = pkgs.writeText "lumo-honcho-postgres" ''
    ...
    start_pre() {
      ...
      ${ensureNetwork}
      if ! ${pkgs.podman}/bin/podman image exists pgvector/pgvector:pg15; then
        ${pkgs.podman}/bin/podman pull pgvector/pgvector:pg15 >&2
      fi
    }
  '';
  ...
  redisService = pkgs.writeText "lumo-honcho-redis" ''
    ...
    supervisor=supervise-daemon
    command="${pkgs.podman}/bin/podman"
    command_args="run --replace --rm --name=lumo-honcho-redis --network=${network} -v ${dataDir}/redis:/data redis:8.2"
    command_user="root"
    ...
    start_pre() {
      ...
      ${ensureNetwork}
      if ! ${pkgs.podman}/bin/podman image exists redis:8.2; then
        ${pkgs.podman}/bin/podman pull redis:8.2 >&2
      fi
    }
  '';
...
in
{
  ...
  home.activation.lumoHoncho = lib.hm.dag.entryAfter [ "lumoDirectories" "sopsAlpine" ] ''
