...
Host output wiring is generated by the local framework under `lib/flake/` from
per-host specs in `hosts/<name>/host.nix`. Host modules remain under
`hosts/<name>/configuration/` and `hosts/<name>/home/`.
